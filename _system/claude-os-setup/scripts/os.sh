#!/usr/bin/env bash
# os.sh — create a new OS folder at a specified location.
#
# Usage: os.sh <OS_Folder_Name> [parent-path]
#
#   <OS_Folder_Name>  — the folder name (e.g. Consulting_OS, Engineering_OS)
#   [parent-path]     — where to create it. Defaults to current directory.
#
# After running, you'll have:
#   <parent-path>/<OS_Folder_Name>/
...
if [ -z "$NAME" ]; then
  echo "Usage: $0 <OS_Folder_Name> [parent-path]" >&2
  echo "Example: $0 Engineering_OS /Users/hamis/Work" >&2
  exit 1
fi
...
if [ -e "$OS_DIR" ]; then
  ...
  echo "To install the .claude/ layer into an existing OS, run:" >&2
...
echo "Next: drop agent .md files into 00_agents/ and re-run install-os-claude.sh"
echo "Or invoke agent-architect to seed the roster."
