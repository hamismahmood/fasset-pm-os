---
type: meeting
name: IBAN Standup
cadence: daily
time: 12:45 PKT
duration_minutes: 30
organizer: Taha Farooq
attendees: [Taha Farooq, Muhammad Zain, Adnan Khan, Faseeh Abdullah, Muhammad Tayyab, Hamis Mahmood, Muhammad Saqlain, Ubaid Rajput, Fazal Erabbi, Muhammad Irfan, Areeb Siddiqui, Salman Khan]
status: active
last_verified: 2026-05-22
tags: [iban, eng-sync, daily]
---

## Purpose
IBAN engineering team alignment — Keel/Due integration, money movement, webhooks, transaction debugging.

## Recurring agenda
- Per-engineer status update.
- Open Keel/Due issues.
- Blockers and dependencies.

## Notes
- Hamis attends as PM owner of the scope (post-Tahir handover).
- Held over Google Meet.
